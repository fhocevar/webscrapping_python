...
def consulta_empresas(empresa_list):
    ...
    for nome_empresa in empresa_list:
        url = f'https://maps.googleapis.com/maps/api/place/textsearch/json?query={nome_empresa}+Herbalife&key={api_key}'
    ...
    with open('empresas_herbalife.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['Nome', 'Endereço', 'Telefone', 'CNPJ'])  # Cabeçalho
        writer.writerows(dados_empresas)  # Escreve os dados das empresas

    print(f"Dados salvos em empresas_herbalife.csv com {len(dados_empresas)} registros.")

# Exemplo de lista de tipos de empresas que vendem produtos Herbalife
empresas = ["loja", "atacado", "nutrição", "suplementos", "saúde", "Herbalife"]
