...
def consulta_empresas(empresa_list, cidades_list):
    ...
    for i, linha in enumerate(dados_empresas, start=1):  # Começa na linha 1 (linha 0 é o cabeçalho)
        # Garantindo que os dados fiquem em uma única célula por coluna
        sheet.write(i, 0, linha[0].strip())  # Nome na coluna 0
        sheet.write(i, 1, linha[1].strip())  # Endereço na coluna 1
        sheet.write(i, 2, linha[2].strip())  # Telefone na coluna 2

    # Salva o arquivo
    arquivo_nome = 'empresas_multicidades_teste_sem_quebra.xls'
...
# Lista de cidades
cidades = ["brasilia", "sao paulo", "porto alegre", "curitiba", "joinville", "florianopolis", "santarem", "riodejaneiro", "saogoncalo", "belem", "salvador", "aracaju", "maceio", "manaus", "campogrande", "portovelho", "riobranco", "campinas", "guarulhos", "cuiaba", "belo horizonte", "uberlandia", "uberaba", "teresina", "recife", "macapa" , "imperatriz" , "gramado" , "maringa" , "fozdoiguacu"]
# Chama a função
