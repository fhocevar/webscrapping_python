...
def consulta_empresas(empresa_list):
    ...
    for nome_empresa in empresa_list:
        # Atualização para buscar em Recife, PE
        url = f'https://maps.googleapis.com/maps/api/place/textsearch/json?query={nome_empresa}+Recife+PE&key={api_key}'
    ...
    with open('empresas_Recife.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['Nome', 'Endereço', 'Telefone', 'CNPJ'])  # Cabeçalho
        writer.writerows(dados_empresas)  # Escreve os dados das empresas

    print(f"Dados salvos em empresas_Recife.csv com {len(dados_empresas)} registros.")
