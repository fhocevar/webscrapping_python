...
def consulta_empresas(empresa_list):
    ...
    for nome_empresa in empresa_list:
        # Altera a consulta para "Florianópolis - SC"
        url = f'https://maps.googleapis.com/maps/api/place/textsearch/json?query={nome_empresa}+Florianópolis+SC&key={api_key}'
    ...
    with open('empresas_florianopolis.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['Nome', 'Endereço', 'Telefone', 'CNPJ'])  # Cabeçalho
        writer.writerows(dados_empresas)  # Escreve os dados das empresas

    print(f"Dados salvos em empresas_florianopolis.csv com {len(dados_empresas)} registros.")
